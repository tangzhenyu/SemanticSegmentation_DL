...
class ResNet_segmentation(object):
	...
	def build_encoder(self):
		...
		with tf.variable_scope(scope_name) as scope:
			...
			with tf.variable_scope('block3') as scope:
				...
				num_layers_block3 = 23 if self.encoder_name == 'res101' else 6   #resnet 101 or resnet 50
	...
	def _ASPP(self, x, num_o, dilations):
		o = []
		for i, d in enumerate(dilations): #applying different dilated rate
			o.append(self._dilated_conv2d(x, 3, num_o, d, name='aspp/conv%d' % (i+1), biased=True))
		return self._add(o, name='aspp/add') #parallel added these feature maps
	...
	def _conv2d(self, x, kernel_size, num_o, stride, name, biased=False):
		...
		Conv2d without BN or relu.
		"""
		num_x = x.shape[self.channel_axis].value #get channel nums
		with tf.variable_scope(name) as scope:
			w = tf.get_variable('weights', shape=[kernel_size, kernel_size, num_x, num_o])
			s = [1, stride, stride, 1]
			o = tf.nn.conv2d(x, w, s, padding='SAME')
